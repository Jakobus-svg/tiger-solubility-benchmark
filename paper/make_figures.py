...
DEEP="#D95F0E"; DEEP2="#E8943F"; DEEP3="#B03A2E"; SIMPLE="#2C7FB8"; SIMPLE2="#5AB4AC"; CALIB="#7B5EA7"
...
# ---- SoluProt: NetSolP / RP3Net / SWI / naive / ProteinSol / PLM_Sol ----
ax=axes[1]
l2=["NetSolP","RP3Net","SWI","Heuristic\n(naive)","ProteinSol\n(eSOL-calib.)","PLM_Sol\n(n=1,994)"]
v2=[0.633,0.619,0.598,0.581,0.542,0.564]; lo2=[0.612,0.599,0.578,0.562,0.521,0.539]; hi2=[0.652,0.640,0.617,0.601,0.562,0.589]
c2=[DEEP,DEEP2,SIMPLE2,SIMPLE,CALIB,DEEP3]
e2=[[v-l for v,l in zip(v2,lo2)],[h-v for v,h in zip(v2,hi2)]]
b2=ax.bar(range(6),v2,0.66,yerr=e2,capsize=2.6,color=c2,error_kw=dict(ecolor="#222",lw=1.0))
labels_above(ax,b2,v2,hi2,fs=7.2)
ax.axhline(0.5,ls=":",c="#999",lw=1)
bracket(ax,0,1,0.690,"n.s.",fs=7.4,color="#666")
ax.set_xticks(range(6)); ax.set_xticklabels(l2,fontsize=6.6)
