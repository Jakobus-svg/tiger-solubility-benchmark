...
// ---------- helpers ----------
const FONT = "Calibri";
// Italicise binomial species names (E. coli / Escherichia coli) inside any run.
const SPECIES = /(Escherichia coli|E\. coli)/g;
const sp = (text, o = {}) => String(text).split(SPECIES).filter(s => s !== "")
  .map(s => (s === "Escherichia coli" || s === "E. coli")
    ? new TextRun({ text: s, italics: true, ...o })
    : new TextRun({ text: s, ...o }));
const body = (runs, opts = {}) => new Paragraph({
  alignment: AlignmentType.JUSTIFIED,
  spacing: { after: 160, line: 276 },
  ...opts,
  children: (typeof runs === "string" ? sp(runs) : Array.isArray(runs) ? runs : [runs]).flat(),
});
const t = (text, o = {}) => sp(text, o);
const b = (text) => sp(text, { bold: true });
...
// numbered references
const refItem = (text) => new Paragraph({
  spacing: { after: 80 }, indent: { left: 360, hanging: 360 },
  children: sp(text, { size: 20 }) });
...
children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 120 },
  children: sp("Published benchmark AUROC does not predict generalisation: an independent evaluation of protein solubility predictors on native and heterologous E. coli proteins", { bold: true, size: 32 }) }));
children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 40 },
  children: t("Jakob Oeschey", { size: 22 }) }));
children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 40 },
  children: t("MSc student, Hochschule Bremen (HSB) – City University of Applied Sciences, Bremen, Germany", { size: 20, italics: true }) }));
children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 200 },
  children: t("Correspondence: joeschey@stud.hs-bremen.de; tiger-biotech@pm.me", { size: 20 }) }));

// Abstract
children.push(H1("Abstract"));
children.push(body([
  b("Motivation. "),
  t("Sequence-based predictors of recombinant protein solubility are widely used to triage expression targets, and deep-learning models report strong benchmark AUROCs. Whether a model's headline benchmark predicts how it ranks on an independent distribution, and how much two state-of-the-art models can differ on the same proteins, is rarely tested directly. Here we show, across two Escherichia coli distributions with per-model leakage control, that a model's published benchmark AUROC is a poor guide to its rank on a new distribution, and that a simple interpretable baseline can match or outperform state-of-the-art deep models outside their training distribution. "),
  b("Results. "),
  t("We benchmarked two state-of-the-art deep models, RP3Net (ESM-2 650M, reported AUROC 0.83) and NetSolP (ESM1b, reported AUROC 0.76), against two dataset-naive composition baselines (an in-house heuristic and the published Solubility-Weighted Index, SWI) on two "),
  i("E. coli"),
  t(" benchmarks, eSOL (native K-12 proteins) and the SoluProt held-out test set (heterologous constructs), with per-model MMseqs2 leakage control. On native cytoplasmic proteins the two deep models spanned the whole range and "),
  t("straddled the baselines"),
  t(": NetSolP was the best predictor (0.792) and RP3Net the worst (0.709), a gap of 0.083 (DeLong p < 1e-8) larger than any model-to-baseline difference. Despite its 0.83 headline, RP3Net was beaten by the simple SWI score (0.745, p = 0.006) and only tied the naive heuristic (0.725), while NetSolP beat SWI (p < 0.001). On the harder heterologous SoluProt set the two deep models instead agreed and led numerically (NetSolP 0.633, RP3Net 0.619, difference not significant), all predictors in a narrow 0.58\u20130.63 band; ProteinSol, calibrated on native eSOL, collapsed to near-chance there (0.542). A third deep model, PLM_Sol (ProtT5), sharpened the contrast further: our pipeline reproduced its ≈ 0.83 own-benchmark AUROC, yet it fell to 0.564 on SoluProt, below every baseline, a value unchanged by removing the 35.7% of the test set that overlapped its training data (identical 0.562 on the overlapping proteins), so a genuine distribution effect rather than memorisation. The native ordering (NetSolP > SWI > RP3Net) survived a homology-level screen (MMseqs2 clustering at \u226430% identity; 8.5% of eSOL removed), confirming it is not a residual-training-homology artefact. "),
  b("Conclusion. "),
...
children.push(body("Recombinant protein production in Escherichia coli is central to molecular biology and biomanufacturing, and insolubility (misfolding into inclusion bodies) is among its commonest failure modes. Many sequence-based predictors triage targets beforehand: physicochemical scores such as ProteinSol, CamSol (Sormanni et al., 2015) and the Solubility-Weighted Index; supervised models such as SoluProt, DeepSol (Khurana et al., 2018) and NetSolP; and predictors built on protein language models such as RP3Net and PLM_Sol."));
...
children.push(body([
  ...
  t("), used here only as a deliberately simple reference, not as a validated predictor. To our knowledge this is the first independent evaluation of RP3Net on eSOL and on the SoluProt test set (its introducing paper reports only AstraZeneca and Structural Genomics Consortium sets). We find that model choice, not model class, dominates on native proteins."),
]));
...
children.push(body([
  b("PLM_Sol (deep, ProtT5). "),
  t("PLM_Sol (Zhang et al., 2024) is a ProtT5-XL protein-language-model embedding model with a BiLSTM–TextCNN head, trained on the updated UESolDS solubility dataset and reporting AUROC ≈ 0.83 on its own held-out test set. UESolDS is an aggregate solubility-in-E.-coli dataset that combines eSOL with TargetTrack, DNASU and PDB-derived records; TargetTrack is the same structural-genomics target pool from which the SoluProt test set is built, so UESolDS is a superset of eSOL but by no means limited to native proteins. We reproduced its released ProtT5 embedding pipeline and inference code on the SoluProt test set; as a positive control the same pipeline reproduced the paper's own benchmark (AUROC 0.834 on its released test set), confirming a faithful re-run. Because UESolDS contains eSOL, PLM_Sol is circular on native eSOL; it is therefore evaluated only on the heterologous SoluProt set, and only after removing the large UESolDS overlap documented in §2.3."),
]));
children.push(body([
...
children.push(body([
  t("Each test set was screened against each model's public training sequences with MMseqs2 (Steinegger & Söding, 2017; easy-search, local alignment; leak = best hit \u226590% identity over \u226580% query coverage). For eSOL: 0.13% (4/3,131) overlapped RP3Net's SGC training and 4.06% (127/3,129) overlapped the PSI:Biology training shared by NetSolP and SWI; all native-protein results use the doubly-clean intersection (n = 3,000 scored; 2,154 cytoplasmic; two b-numbers mapped many-to-one from eSOL and were de-duplicated). For the SoluProt test set: 1.29% (40/3,100) overlapped RP3Net's training, leaving 3,060 clean proteins; separately, 4.02% (123/3,060) of SoluProt proteins were near-identical to eSOL (mostly 100% identity; the “heterologous” set contains some native E. coli sequences), removed for the ProteinSol evaluation (n = 2,937). PLM_Sol required by far the largest correction of any model: 35.68% (1,106/3,100) of the SoluProt test set was near-identical to its UESolDS training data, leaving 1,994 leak-free proteins for its evaluation. This nine-fold gap between SoluProt's overlap with UESolDS (35.7%) and with eSOL alone (4.0%) is itself informative, and the matched training sequences are overwhelmingly tagged structural-genomics constructs (NESG/JCSG/MCSG/pET-vector entries): it shows UESolDS draws heavily on the same structural-genomics repository (TargetTrack) that the SoluProt test set is built from, so two nominally distinct benchmarks share a large fraction of identical sequences, precisely the hidden train/test overlap that per-model screening is designed to remove. Here the screen is a control rather than a correction: PLM_Sol scored essentially the same on the overlapping and the non-overlapping proteins (AUROC 0.562 vs 0.564), so its weak SoluProt performance is a genuine distribution effect and not a leakage artefact; we nonetheless report only the leak-free subset as the conservative protocol. Proprietary training portions (RP3Net's AstraZeneca data) cannot be screened, but are drug-discovery targets unlikely to overlap these native/structural-genomics sets. This is a near-duplicate screen (\u226590% identity); it removes near-identical sequences but not homologs, so residual familiarity between a test set and a model\u2019s broader training distribution cannot be excluded. We therefore also ran a homology-level screen: each eSOL query was clustered with the PSI:Biology training sequences (MMseqs2 easy-cluster, min-seq-id 0.30, coverage 0.5), and a query sharing a cluster with any training sequence was treated as a homolog leak. This removed 256/3,000 eSOL proteins (8.5%), versus 4.06% at the \u226590% near-duplicate threshold; all native-protein comparisons were re-run on the homology-clean subset (n = 1,929 cytoplasmic, \u00a73.1). All screens used a single MMseqs2 build (commit 8cc5ce3); re-running the three near-duplicate screens at maximum sensitivity (-s 7.5) reproduced identical leak sets (eSOL vs RP3Net 4/3,131; SoluProt vs RP3Net 40/3,100; eSOL vs PSI:Biology 127/3,129), confirming the screens are not sensitivity-limited."),
]));
children.push(H2("2.4 Statistical analysis"));
children.push(body([
  t("AUROC via the Mann-Whitney statistic; 95% CIs by case-resampling bootstrap (2,000 resamples). Two predictors on the same proteins were compared by the DeLong test (1988) as the primary significance test and, independently, a paired bootstrap of the AUROC difference as a robustness cross-check (the bootstrap achieved-significance-level cannot resolve p below 1/2,000, so small p-values are reported from DeLong). Significance requires a paired interval excluding zero with concordant DeLong p; \u201cn.s.\u201d is read from an interval spanning zero. Threshold robustness was assessed at 60/70/80%. Predictor scores were used at their native resolution \u2014 NetSolP, SWI and ProteinSol as continuous outputs, RP3Net and the composition heuristic as their integer-rescaled score; the coarser binning attenuates the latter two AUROCs by \u2264 0.002 (verified on SoluProt, where RP3Net\u2019s continuous probability gives 0.620 vs 0.619 binned) and changes no ranking or significance verdict. As a threshold-free complement to AUROC on eSOL, we also report Spearman\u2019s \u03c1 between each predictor\u2019s score and the continuous solubility percentage (\u00a73.2). Across the six pre-specified primary comparisons (the homology-clean re-runs in \u00a73.1 are confirmatory robustness checks of the same hypotheses, not additional tests), the significant native-protein findings survive Bonferroni correction over the six tests (NetSolP vs RP3Net and NetSolP vs SWI, adjusted p < 0.005; SWI vs RP3Net, p = 0.006, Bonferroni-adjusted p = 0.036; Holm's step-down is uniformly no more conservative)."),
]));
...
children.push(body([
  b("Threshold-free confirmation. "),
  t("Because eSOL reports a continuous solubility percentage, we also correlated each predictor's score with the measured solubility directly (Spearman's ρ, no binarisation). The ordering was unchanged: ρ = 0.54 (NetSolP), 0.46 (SWI), 0.42 (RP3Net) and 0.40 (naive heuristic), so NetSolP's lead and RP3Net's tie with the naive baseline are not artefacts of the 70% cutoff."),
]));
children.push(body([
...
children.push(body([
  t("On the heterologous SoluProt test set (leak-free, n = 3,060) the picture inverted: the two deep models clustered at the top and were statistically indistinguishable from each other (NetSolP 0.633, 0.612\u20130.652; RP3Net 0.619, 0.599\u20130.640; paired \u0394 +0.014, p = 0.23), both numerically above the simple baselines (SWI 0.598; naive heuristic 0.581). Only NetSolP significantly exceeded SWI here (\u0394 +0.035, p \u2248 0.002); RP3Net significantly beat the naive heuristic (\u0394 +0.038, p = 0.005) but was indistinguishable from SWI (\u0394 +0.021, p = 0.073), and all four AUROCs lie in a narrow 0.58\u20130.63 band where no predictor is strongly discriminative. On this harder distribution, then, the deep models behaved alike and led numerically, the opposite of their wide divergence on native proteins."),
]));
children.push(body([
...
children.push(body([
  b("A third deep model breaks with the others. "),
  t("Not every language-model predictor tracked the two leading deep models here. PLM_Sol (ProtT5; Zhang et al., 2024), which reports AUROC ≈ 0.83 on its own updated-eSOL benchmark and whose pipeline we reproduced exactly on that benchmark (0.834, our re-run), fell to AUROC 0.564 (95% CI 0.539–0.589) on the leak-free SoluProt set (n = 1,994, after removing the 35.68% of SoluProt that was near-identical to its UESolDS training data, the largest training overlap of any model here). Critically, this overlap did not inflate the score: PLM_Sol performed identically on the 1,106 overlapping proteins (0.562) and the 1,994 non-overlapping ones (0.564), so the low value is a genuine distribution effect, not memorised training labels; the leakage screen is a control here, not the cause of the collapse. At 0.564 PLM_Sol ranked below every other predictor, including both simple baselines, and was statistically indistinguishable from the naive composition heuristic (paired Δ −0.022, DeLong p = 0.15) and from ProteinSol (Δ +0.010, p = 0.49). It was significantly beaten by NetSolP (Δ −0.056, p = 0.0004), SWI (Δ −0.040, p = 0.009) and RP3Net (Δ −0.036, p = 0.02) on the shared proteins (n = 1,969; ProteinSol comparison n = 1,907). A ~0.83-benchmark model thus collapsed to its own naive floor on an independent distribution, the sharpest single instance in this study of a headline AUROC failing to predict generalisation."),
]));
children.push(H2("3.4 Threshold robustness and SoluProt leakage"));
children.push(body([
  t("Re-binarising eSOL at 60/70/80% left the cytoplasmic ordering unchanged (Fig. 2). The SoluProt\u2013RP3Net leakage screen removed 40/3,100 (1.29%) training-overlapping proteins; this lowered RP3Net\u2019s SoluProt AUROC from 0.621 to 0.619 and turned its nominally significant edge over SWI on the full set (p = 0.044) into a non-significant one leak-free (p = 0.073), a concrete illustration that even 1\u20132% overlap must be removed before reporting marginal differences."),
]));
...
children.push(...img("fig1_auroc.png", 500, 245, [
  b("Figure 1. "),
  t("AUROC (95% CI), all leak-free. Left: on native cytoplasmic eSOL proteins the two deep models (orange) occupy the extremes (NetSolP best 0.792, RP3Net worst 0.709), straddling the two simple interpretable baselines (blue: SWI 0.745, naive heuristic 0.725); SWI significantly beats RP3Net (p = 0.006). Right: on heterologous SoluProt proteins the two leading deep models lead and agree (n.s.), both above the baselines. ProteinSol (purple, calibrated on native eSOL) collapses to near-chance (0.542) on this heterologous set, and PLM_Sol (dark red, ProtT5; n = 1,994 after removing its 35.7% training overlap) falls to 0.564, below every baseline despite ≈ 0.83 on its own benchmark.", { size: 18 }),
]));
...
children.push(H2("Table 2. Paired comparisons on shared proteins (leak-free)"));
{
  ...
  children.push(new Table({ width: { size: 9000, type: WidthType.DXA }, columnWidths: W,
    rows: [
      ...
      row(["SoluProt: NetSolP vs PLM_Sol", "1,969", "+0.056 [+0.024, +0.086]", "0.0006 / 0.0004", "NetSolP \u226b PLM_Sol"], W, { fill: "FBEEE6" }),
      row(["SoluProt: SWI vs PLM_Sol", "1,969", "+0.040 [+0.011, +0.071]", "0.009 / 0.009", "SWI > PLM_Sol"], W, { fill: "FBEEE6" }),
      row(["SoluProt: RP3Net vs PLM_Sol", "1,969", "+0.036 [+0.006, +0.066]", "0.02 / 0.02", "RP3Net > PLM_Sol"], W, { fill: "FBEEE6" }),
      row(["SoluProt: PLM_Sol vs naive heuristic", "1,969", "\u22120.022 [\u22120.052, +0.007]", "0.14 / 0.15", "indistinguishable"], W, { fill: "FBEEE6" }),
    ] }));
  children.push(body([ i("Native (eSOL) comparisons on the doubly leak-free intersection (n = 2,154 cytoplasmic). The two leading deep models span 0.083 AUROC on native proteins yet are indistinguishable on heterologous proteins. PLM_Sol comparisons use its own leak-free intersection with each predictor (n = 1,969), reflecting its larger 35.7% training overlap with SoluProt.") ], { spacing: { before: 80, after: 200 } }));
}
...
children.push(body([
  ...
  t("Native E. coli cytoplasmic proteins are co-adapted to the host’s folding, chaperone and codon environment, and eSOL measures them without expression tags. The SoluProt constructs are heterologous, typically carry purification tags, and span signal-peptide-bearing, membrane-associated and intrinsically disordered proteins whose solubility depends on features (long hydrophobic spans, disorder, host-mismatched codon usage) that differ from native cytoplasmic determinants. A predictor that keys on one regime’s features need not transfer to the other, a plausible route to the model- and distribution-dependence seen here."),
]));
children.push(body([
  b("Benchmarks do not predict generalisation. "),
  t("RP3Net reports 0.83 on its prospective industrial set yet was ranked lowest of four predictors on native E. coli proteins and indistinguishable from the best only on heterologous ones. A single headline AUROC, measured on one distribution, did not rank the models on another. PLM_Sol makes the point most starkly: it reproduces a ≈ 0.83 benchmark on its own updated-eSOL test set (which we confirmed by re-running its released pipeline, 0.834) yet collapses to 0.564 on leak-free SoluProt, below every simple baseline and indistinguishable from a naive composition score, a 0.27-AUROC gap between a model's advertised number and its performance on an independent set. Although 36% of SoluProt overlapped PLM_Sol's training, that overlap did not inflate its score (it performed identically on the overlapping and non-overlapping proteins, 0.562 vs 0.564), so the collapse is a clean distribution effect with no leakage confound. This is not a criticism of any one model (each may be well-suited to its own target distribution) but a caution against selecting a solubility predictor by its reported number."),
]));
children.push(body([
  b("Consistency with prior benchmarks. "),
  t("Our results do not contradict the benchmarks on which these models were introduced; they extend them to an untested distribution. RP3Net reports AUROC 0.83 and roughly a 0.08 lead over NetSolP on its own prospective set of recombinant constructs (human drug targets assayed at AstraZeneca; Tankhilevich et al., 2026), and NetSolP reports about 0.76 on its own independent validation set (Thumuluri et al., 2022); each headline is measured on a distribution close to its own training data, and both are heterologous recombinant proteins, the regime these tools are built for. Neither was measured on native E. coli proteins. On the heterologous SoluProt set we likewise find the two deep models comparable and leading the baselines, consistent with RP3Net's design-domain strength; its published lead over NetSolP on recombinant constructs is untouched by our native-protein result. eSOL is not that use case (it measures native, tag-free, chaperone-free solubility), and we use it not as the target task but as an out-of-domain generalisation probe. Read this way, RP3Net's lower native ranking is the expected signature of a model evaluated outside its design domain, not evidence against its reported performance."),
]));
children.push(body([
  b("Consistency with independent benchmarks. "),
  t("Where our setup overlaps a published one, the numbers replicate (Table 3). On the SoluProt test set our SWI and ProteinSol AUROCs (0.598, 0.542) reproduce the SoluProt paper's own figures (0.60, 0.54; Hon et al., 2021), and on eSOL our SWI rank-correlation (Spearman ρ = 0.46, cytoplasmic) matches the SWI paper's eSOL value (ρ = 0.50; Bhandari et al., 2020), an independent check that the pipeline and datasets are the intended ones. As a further positive control, re-running PLM_Sol's released ProtT5 pipeline on its own held-out test set reproduced its benchmark AUROC (0.834, versus ≈ 0.83 reported; Zhang et al., 2024), confirming the collapse to 0.564 on SoluProt is a genuine distribution effect and not a pipeline error on our side. The comparison also underscores the central point: NetSolP's own reported AUROC ranges from 0.48 on SGC Stockholm (Tankhilevich et al., 2026), through ~0.62 on a re-curated E. coli solubility set (Zhang et al., 2024), 0.73 on its PSI:Biology cross-validation and 0.76 on the independent Price set (Thumuluri et al., 2022), to 0.79 here on classic eSOL, the same model spanning 0.48–0.79 across datasets. Absolute AUROCs and cross-tool rankings are thus curation-dependent (on Zhang et al.'s homology-split set the tools score lower and re-order), while the direction we rely on, NetSolP > SWI on native proteins, holds in every independent report that includes both."),
]));
{
  const W = [3400, 1500, 1500, 2600];
  children.push(H2("Table 3. Consistency with independent benchmarks"));
  children.push(new Table({ width: { size: 9000, type: WidthType.DXA }, columnWidths: W,
    rows: [
      tableHeader(["Cross-check", "This work", "Reported", "Source"], W),
      row(["SWI, SoluProt (AUROC)", "0.598", "0.60", "Hon et al. 2021"], W),
      row(["ProteinSol, SoluProt (AUROC)", "0.542", "0.54", "Hon et al. 2021"], W),
      row(["SWI, eSOL (Spearman ρ)", "0.46", "0.50", "Bhandari et al. 2020"], W),
      row(["PLM_Sol, own test set (AUROC)", "0.834", "≈ 0.83", "Zhang et al. 2024"], W),
      row(["NetSolP, across datasets (AUROC)", "0.63 / 0.79", "0.48–0.76", "Tankhilevich 2026; Zhang 2024; Thumuluri 2022"], W),
    ] }));
  children.push(body([ i("Direct cross-checks: our SoluProt SWI/ProteinSol AUROCs, our eSOL SWI Spearman, and our re-run of PLM_Sol on its own test set all reproduce the published values. The NetSolP row shows this work (SoluProt 0.63 / eSOL 0.79) against the range of independently reported NetSolP AUROCs (0.48 SGC Stockholm; ~0.62 re-curated E. coli set; 0.64 AZ; 0.73 PSI:Biology CV; 0.76 Price), the same model spanning 0.48–0.79 across datasets.") ], { spacing: { before: 80, after: 160 } }));
}
children.push(body([
  b("What these predictors actually target. "),
  t("A terminology note underlies all of the above: RP3Net and NetSolP (like SoluProt and the other PSI:Biology-derived tools) are trained on soluble recombinant expression outcomes, not on intrinsic thermodynamic solubility. eSOL (native, untagged, measured in a chaperone-free reconstituted system) sits nearer the intrinsic-solubility end, which is precisely what makes it a demanding out-of-domain probe rather than a home benchmark for these models. The field's shorthand “solubility predictor” thus spans two related but distinct targets, and the distribution-dependence we report is in part a consequence of that gap."),
]));
children.push(body([
...
[
  ["Scope of models. ", "We benchmark three deep models (RP3Net, NetSolP on both distributions; PLM_Sol on SoluProt), a native-calibrated predictor (ProteinSol) and two simple baselines; the qualitative point (large between-model variance; benchmark does not predict generalisation) may not capture every predictor, and adding further models (DeepSol, CamSol, ccSOL) would sharpen the variance estimate."],
...
children.push(body([
  ...
  t("The author develops TiGer Biotech, a freely available, non-commercial sequence-based expression-feasibility tool whose composition heuristic is used here as one of the baselines. The tool is not commercialised; its development is nonetheless disclosed here as a potential professional interest. To limit its influence, the in-house heuristic is treated only as a corroborating baseline and never carries a conclusion on its own; the externally published Solubility-Weighted Index (SWI) serves as the primary, more conservative baseline, and the full (eSOL-fit) heuristic variant is explicitly excluded from all claims. RP3Net, NetSolP, SWI, ProteinSol and PLM_Sol are third-party tools used as released, without involvement of their authors in this work. The author has no other competing interests."),
]));
children.push(body([
  b("Funding. "),
  t("none."),
...
[
  ...
  "Khurana S, Rawi R, Kunji K, Chuang G-Y, Bensmail H, Mall R (2018). DeepSol: a deep learning framework for sequence-based protein solubility prediction. Bioinformatics 34(15):2605\u20132613. doi:10.1093/bioinformatics/bty166.",
  ...
  "Thumuluri V, Martiny HM, Almagro Armenteros JJ, Salomon J, Nielsen H, Johansen AR (2022). NetSolP: predicting protein solubility in Escherichia coli using language models. Bioinformatics 38(4):941\u2013946. doi:10.1093/bioinformatics/btab801.",
  ...
  "Zhang X, Hu X, Zhang T, Yang L, Liu C, Xu N, Wang H, Sun W (2024). PLM_Sol: predicting protein solubility by benchmarking multiple protein language models with the updated Escherichia coli protein solubility dataset. Brief Bioinform 25(5):bbae404. doi:10.1093/bib/bbae404.",
].forEach(r => children.push(refItem(r)));
// ---------- document ----------
const doc = new Document({
  creator: "Jakob Oeschey",
  title: "Published benchmark AUROC does not predict generalisation: an independent evaluation of protein solubility predictors",
  styles: {
