...
def best_hits(m8: Path, id_thr: float, cov_thr: float) -> dict:
    """query b-number -> (leak, pident%, qcov, tcov, target).

    leak = True iff ANY hit for this query clears both thresholds (>= id_thr% identity AND
    >= cov_thr query coverage). The stored hit is the highest-identity qualifying hit when one
    exists, else the highest-identity hit (for reporting). This fixes an earlier version that kept
    only the single max-identity hit and could miss a leak whose qualifying hit had a lower identity
    but sufficient coverage (e.g. 95%/0.60 masking a real 92%/0.85 leak)."""
    best = {}
    with open(m8) as fh:
        for line in fh:
            p = line.rstrip("\n").split("\t")
            ...
            qualifies = pid >= id_thr and qcov >= cov_thr
            cur = best.get(q)
            if cur is None or (qualifies, pid) > (cur[0], cur[1]):
                best[q] = (qualifies, pid, qcov, tcov, t)
    return best
...
def main():
    ...
    total = len(esol)
    leaks = sorted(
        [(q, pid, qc, t) for q, (ql, pid, qc, tc, t) in best.items() if ql],
        key=lambda x: -x[1])
